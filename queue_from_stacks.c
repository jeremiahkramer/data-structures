...
int queue_from_stacks_isempty(struct queue_from_stacks* queue) {
	assert(queue);
	if(stack_isempty(queue->s1) == 1 && stack_isempty(queue->s2) == 1) //if both stacks are empty return 1
		return 1;
	else
		return 0;
}
...
int queue_from_stacks_front(struct queue_from_stacks* queue) {
	...
	if (stack_isempty(queue->s2) == 1){ 
		while(stack_isempty(queue->s1) == 0){ //loop until empty
			val = stack_pop(queue->s1);	 //pop off top
			stack_push(queue->s2, val); //push onto other stack
		}
	}
	val = stack_top(queue->s2); //sorted so use top function to peek
	return val;
}
...
int queue_from_stacks_dequeue(struct queue_from_stacks* queue) {
	...
	val = stack_pop(queue->s2); //sorted so use pop function to dequeue
	return val;
}
